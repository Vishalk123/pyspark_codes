...
ned_df = orderdf.withColumnRenamed("order_customer_id","customer_id")
join_condition = orderdf["customer_id"] == customerdf["customer_id"]
join_type = "outer"
# using coalesce for handling null for null order_id i will print -1
joined_df = ned_df.join(customerdf,join_condition,join_type)
# joined_df.show()
# stdin.readline()
spark.stop()
